...
impl Hand {
    ...
    fn get_type(&self) -> HandType {
        ...
        // if there is a joker than add the joker count to the highest card count
        if counts.contains_key(&Card::Joker) {
            let joker_count = counts[&Card::Joker];
            if joker_count == 5 {
            ...
            let most_frequent_card = counts
                ...
                .unwrap();
            let most_frequent_card_entry = counts.entry(*most_frequent_card.0).or_insert(0);
            *most_frequent_card_entry += joker_count;
